...
#include "bosscleaner.h"
#include "../character.h"
#include "../projectile.h"

MACRO_ALLOC_POOL_ID_IMPL(CBossCleaner, MAX_CLIENTS * ENGINE_MAX_MAPS + MAX_CLIENTS)

CBossCleaner::CBossCleaner(CGameWorld *pWorld)
	: CCharacter(pWorld)
{
	m_BotDir = 1;
	m_BotLastPos = m_Pos;
...
void CBossCleaner::Tick()
{
	TickBotAI();
	CCharacter::Tick();
}

bool CBossCleaner::TakeDamage(vec2 Force, int Dmg, int From, int Weapon)
{
