...
MACRO_ALLOC_POOL_ID_IMPL(CBossPig, MAX_CLIENTS * ENGINE_MAX_MAPS + MAX_CLIENTS)
...
void CBossPig::TickBotAI()
{
	...
	if (m_SummonTick <= 0)
	{
		...
		for (int i = MAX_PLAYERS; i < MAX_CLIENTS; i++)
		{
            ...
            int SummonMonster = GameServer()->GetBossLeveling() > 400 ? BOT_L3MONSTER : BOT_L1MONSTER;
			// 如果是要召唤的怪物，就把它召唤到Boss战场地
			if (pPlayer->IsBot() && random_prob(0.5f) && !pPlayer->GetCharacter()->m_SummonByBoss && pPlayer->GetBotType() == SummonMonster)
			{
				GameServer()->m_BossSummonNum++;
				pPlayer->GetCharacter()->m_SummonByBoss = true;
				pPlayer->GetCharacter()->m_Core.m_Pos = m_Pos;
				pPlayer->GetCharacter()->m_Core.m_Pos.y -= 12;
